...
def generate_hdf5_file(input_dir, file_name, ouptut_dir):
    if not os.path.isdir(ouptut_dir):
        os.mkdir(ouptut_dir)
    file_path = Path(ouptut_dir + "/" + file_name + ".hdf5")
    hf = h5py.File(file_path, "w")
    img_file_names, img_file_data, labeled_file_data = process_directory(input_dir, ouptut_dir, save_file=False)
